                    ...
                    <section>
                        ...
                        <div className="card" style={{ overflow: 'hidden' }}>
                            {event.sessions.map((session, i) => (
                                <div key={session.id} style={{ padding: '24px', borderBottom: i === event.sessions.length - 1 ? 'none' : '1px solid var(--border)', display: 'grid', gridTemplateColumns: '120px 1fr', gap: '24px' }}>
                                    <div style={{ color: 'var(--text-secondary)', fontWeight: 600 }}>
                                        <div style={{ fontSize: '14px', marginBottom: '4px' }}>Day {session.day}</div>
                                        <div style={{ fontSize: '18px', color: 'var(--text-primary)' }}>{session.time}</div>
                                    </div>
                                    <div>
                                        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '8px' }}>
                                            <h4 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--text-primary)' }}>{session.title}</h4>
                                            <span style={{ padding: '2px 10px', background: 'var(--bg-highlight)', color: 'var(--text-secondary)', borderRadius: '12px', fontSize: '12px' }}>{session.track}</span>
                                        </div>
